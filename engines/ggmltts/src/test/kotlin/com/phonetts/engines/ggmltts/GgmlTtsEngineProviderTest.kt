 ...
 */
class GgmlTtsEngineProviderTest {
    @Test
    fun `provider is discoverable via ServiceLoader on this module's classpath`() {
        ...
        assertEquals(listOf(GgmlTtsEngine.ENGINE_ID), ids)
    }

    @Test
    fun `provider engineId matches the engine it creates`() {
        assertCreatesMatchingEngine(GgmlTtsEngineProvider(), emptyContext())
    }

    @Test
    fun `create builds a working GgmlTtsEngine`() {
        val provider = GgmlTtsEngineProvider()

        val engine = provider.create(emptyContext())

        assertEquals(GgmlTtsEngine.ENGINE_ID, engine.id)
